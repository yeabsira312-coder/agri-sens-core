...
# --- DYNAMIC RECOMMENDATION ENGINE MAPPING ---
# Matches all exact outputs generated by RiskEngine.compute_cari()
RECOMMENDATIONS = {
    # Emergency / Extreme Tiers
    "Severe": (
...
def run_cached_pipeline(
    ...
    # Retrieve secure user-agent header if present in secrets, else fallback safely
...
def display_recommendation(risk_tier: str) -> None:
    """Helper component to dynamically render actionable risk recommendations."""
    tier_clean = str(risk_tier).strip()
    
    # 1. Direct Lookup
    rec_text = RECOMMENDATIONS.get(tier_clean)
    
    # 2. Case-Insensitive / Substring Match (Safety Net)
    if not rec_text:
    ...
    # 3. Default Fallback
    if not rec_text:
        rec_text = f"ℹ️ **Current Status ({risk_tier}):** Crop health is stable. Continue routine soil testing and water management."
